...
function formHasErrors() {
	...
	const requiredTextFields = ["fullname", "address", "city"]
	...
	const postalCodeFormat = new RegExp(/^[ABCEGHJ-NPRSTVXY]\d[ABCEGHJ-NPRSTV-Z][ -]?\d[ABCEGHJ-NPRSTV-Z]\d$/i)
	...
	for (const cardtype of cardtypes) {
		if (document.getElementById(cardtype).checked) {
	...
	//  Checking name on card
	const requiredName = "cardname"

		if (!formFieldHasInput(document.getElementById(requiredName))) {
			document.getElementById(requiredName + "_error").style.display = "block"
	...
	if (cardNumber.length == 10) {
		const sum = cardNumber[0] * 4 + cardNumber[1] * 3 + cardNumber[2] * 2 + cardNumber[3] * 7 + cardNumber[4] * 6 
					+ cardNumber[5] * 5 + cardNumber[6] * 4 + cardNumber[7] * 3 + cardNumber[8] * 2	
		const remainder = sum % 11
		const checkingNumber = 11 - remainder

		if (checkingNumber != cardNumber[9]) {
